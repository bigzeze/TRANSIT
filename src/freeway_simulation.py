...
sce_name = 'freeway_RSL' # RSL: Road Speed Limitation
...
# 1
#realWorldRoadGenerator(current+'/../inputs/freeways.osm',sce_model+'roads.net.xml',verbose=True)
...
# 3
endtime = 7200
highway_generate_routes(sce_model+'roads.net.xml',sce_model+'flows.rou.xml',busPerc,endtime,alpha,True)
...
interface = SUMOInterface(sumoBinary,sce_model+'roads.net.xml',sce_model+'flows.rou.xml',statistic_file,tripinfo_file,verbose=True,additional_file=sce_model+'detectors.add.xml')

interface.closeSUMO()
interface.startSUMO()
# interface.simulation(0,10800,[EdgeSlow(3600,7200,{rsl_target:new_speed_limit})])
# #interface.simulation(0,7200,[TLFailure(1800,5400,['C2'])])
interface.simulation(0,7200,[SpeedLimit(1800,5400,{'28414138':1})])
