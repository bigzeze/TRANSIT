...
sce_name = 'manhattan_RSL' # RSL: Road Speed Limitation
...
# 1
manhattanRoadGenerator(sce_model+'roads.net.xml',5,200,1,True,verbose=True)
manhattanDetectorGenerator(sce_model+'roads.net.xml',sce_model+'detectors.add.xml','detectors.out.xml',60,'center',True,1)

#realWorldRoadGenerator(current+'/map.osm',path+'realworld.net.xml',verbose=True)
#realWorldDetectorGenerator(path+'realworld.net.xml',path+'detectors.add.xml','detectors.out.xml',60,verbose=True)
...
# 3
manhattan_generate_routes(sce_model+'roads.net.xml',flow_files,sce_model+'routes.routes.xml',verbose=True)
...
interface.simulation(0,10800,[SpeedLimit(3600,7200,{'C2D2':new_speed_limit})])
